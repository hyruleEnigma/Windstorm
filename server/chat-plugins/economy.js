// Original code taken from Wavelength (HoeenCoder/Wavelength).
// All credit would be given to the original creator of the command (which I assume to be CreaturePhil) and others who worked on it.
'use strict';
...
exports.commands = {
	...
	wallet: function (target, room, user) {
		...
		Economy.readMoney(userid, money => {
			this.sendReplyBox(target + " has " + money + ((money === 1) ? " " + currencyName + "." : " " + currencyPlural + "."));
	...
	takecurrency: function (target, room, user, connection, cmd) {
		...
		Economy.writeMoney(targetUser, -amount, () => {
			Economy.readMoney(targetUser, newAmount => {
				if (Users.get(targetUser) && Users.get(targetUser).connected) {
					Users.get(targetUser).popup('|html|' + user.userid + ' has removed ' + amount + ' ' + (amount === 1 ? currencyName : currencyPlural) +
					' from you.<br />');
				}
		...
		Economy.readMoney(user.userid, money => {
			...
			if (cmd !== 'confirmtransfercurrency' && cmd !== 'confirmtransferfeathers') {
				return this.popupReply('|html|<center>' +
					...
					'Confirm transfer to <br><b style = "color:' + targetUser + '; text-shadow: 1px 1px 1px rgba(0, 0, 0, 0.8)">' + Chat.escapeHTML(targetUser) + '</b></button></center>'
				);
			}
					...
					Economy.readMoney(targetUser, firstAmount => {
						Economy.readMoney(user.userid, secondAmount => {
							...
							if (Users.getExact(targetUser) && Users.getExact(targetUser).connected) {
								Users.getExact(targetUser).send('|popup||html|' + user.name + " has sent you " + amount + ((amount === 1) ? " " + currencyPlural : " " + currencyPlural));
							}
						});
					});
				});
