...
function sendMessages(str) {
	if (Rooms.get(toID(Config.github.room))) {
		Rooms.get(toID(Config.github.room)).add("|raw|<div class=\"infobox\">" + str + "</div>").update();
	} else {
...
github.on('push', function push(repo, ref, result) {
	...
	result.commits.forEach(function (commit) {
		...
		message += commit.author.name + ": " + Chat.escapeHTML(shortCommit);
		messages.push(message);
	});
